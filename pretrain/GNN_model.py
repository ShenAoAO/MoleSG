...
#         # self_loop_attr = torch.zeros(x.size(0), 13)
#         # self_loop_attr = self_loop_attr.to(edge_attr.device).to(edge_attr.dtype)
#         # self_loop_attr[:,] = torch.nn.Embedding(4, 13)
#         # edge_embeddings = torch.cat((edge_attr, self_loop_attr), dim=0)
#
#
#
...
class GINConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        edge_index, _ = add_self_loops(edge_index, num_nodes=x.size(0))
        self_loop_attr = edge_attr.new_zeros((x.size(0), 13))
        edge_attr = torch.cat((edge_attr, self_loop_attr), dim=0)
        edge_embeddings = self.MLP(edge_attr.float())
